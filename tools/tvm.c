...
main(int argc, char **argv)
{
    ...
    if (numRead != RESPONSE_LENGTH)
    {
      ...
      printf ("Please make sure RCX has TinyVM firmware (0.2.0+) "
    ...
    if (response[5] != (byte) (MAGIC >> 8) || response[7] != (byte) (MAGIC & 0xFF))
    {
      printf ("Unexpected response from RCX. The RCX either doesn't have firmware or "
              "the firmware in it is not TinyVM 0.2.0+ in program-download mode.\n");
      exit (1);
    }
    ...
    do {
      ...
      if (index != 0)
      {
        ...
        if (numRead != 9)
        {
  	  fprintf (stderr, "transfer data: invalid response length. (Firmware version should be 0.2.1+).\n");
	  exit (1);
	}
        if (response[5] != 0)
        {
