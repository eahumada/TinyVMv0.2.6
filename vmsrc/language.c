...
{
  byte counter;
  counter = classRecord->numMethods;
  while (counter--)
  {
    gMethodRecord = get_method_record (classRecord, counter);
                          ...
                          byte *retAddr)
{
  ...
  // Note: the native method could be Thread.yield()
  return true;
}

/**
 * Pops a stack frame and pushes return value on operand stack.
 */
